...
public class RestAssured_TestCases {
    ...
    "PL1" }, description = 
				    "1. Send a GET request to '/web/index.php/api/v2/pim/employees?limit=50&offset=0&model=detailed&includeEmployees=onlyCurrent&sortField=employee.firstName&sortOrder=ASC' with a valid cookie\n"
	...
	public void getEmployeeList() throws IOException {
		...
		CustomResponse customResponse = apiUtil.getEmployeeList("/web/index.php/api/v2/pim/employees?limit=50&offset=0&model=detailed&includeEmployees=onlyCurrent&sortField=employee.firstName&sortOrder=ASC" , cookieValue,
				null);
	    ...
	    boolean isImplementationCorrect = TestCodeValidator.validateTestMethodFromFile(
	            apiUtilPath, "getEmployeeList", List.of("given", "cookie", "get", "response"));
	    System.out.println(isImplementationCorrect);
	...
	public void putLanguagePackage() throws IOException {
		...
		CustomResponse customResponse=apiUtil.putLanguagePackage("/web/index.php/api/v2/admin/i18n/languages/"+id, cookieValue , null);
	    ...
	    // Validate implementation correctness from file (if needed)
	    boolean isImplementationCorrect = TestCodeValidator.validateTestMethodFromFile(
	            apiUtilPath, "putLanguagePackage", List.of("given", "cookie", "put", "response"));
	    System.out.println(isImplementationCorrect);
	...
	public void deleteLangById() throws IOException {
	    ...
	    CustomResponse customResponse = apiUtil.deleteLangById("/web/index.php/api/v2/admin/i18n/languages",cookieValue,requestBody);
	    ...
	    Assert.assertTrue(isImplementationCorrect, "deleteLangById must be implemented using proper RestAssured methods.");
	...
	public void putModulesSettings() throws Exception {
		...
		Map<String, String> TestData = FileOperations.readExcelPOI(
	    	    excelPath,
	    	    "PutData9"
	    	);
		
		String requestBody = "{"
		        + "\"admin\": " + TestData.get("admin").toLowerCase() + ","
		        + "\"buzz\": " + TestData.get("buzz").toLowerCase() + ","
		        + "\"claim\": " + TestData.get("claim").toLowerCase() + ","
		        + "\"directory\": " + TestData.get("directory").toLowerCase() + ","
		        + "\"leave\": " + TestData.get("leave").toLowerCase() + ","
		        + "\"maintenance\": " + TestData.get("maintenance").toLowerCase() + ","
		        + "\"mobile\": " + TestData.get("mobile").toLowerCase() + ","
		        + "\"performance\": " + TestData.get("performance").toLowerCase() + ","
		        + "\"pim\": " + TestData.get("pim").toLowerCase() + ","
		        + "\"recruitment\": " + TestData.get("recruitment").toLowerCase() + ","
		        + "\"time\": " + TestData.get("time").toLowerCase()
		        + "}";

	    // Step 1: Call API
	    CustomResponse customResponse = apiUtil.putModulesSettings(
	        "/web/index.php/api/v2/admin/modules",
	        cookieValue,
	        requestBody
	    );
	    ...
	    Assert.assertTrue(
	        TestCodeValidator.validateResponseFields("putModulesSettings", customResponse),
	...
	public void postOpenIdProvide() throws Exception {
	    ...
	    String requestBody = "{"
	            ...
	            + "\"clientId\": \"" + TestData.get("clientId") + "\","
	            + "\"clientSecret\": \"" + TestData.get("clientSecret") + "\""
	            + "}";

	    // Step 2: Send request
	    CustomResponse customResponse = apiUtil.postOpenIdProvide(
	    ...
	    Assert.assertEquals(
	        ...
	        200,
	        "Expected status code 200 for successful POST."
	    );

	    // Step 6: Optional: Validate expected fields
	    Assert.assertTrue(
	        TestCodeValidator.validateResponseFields("postOpenIdProvide", customResponse),
	        "Response must contain expected OpenID fields."
	    );
	    
	    // Checking For Implementation
		Assert.assertTrue(isImplementationCorrect, "postOpenIdProvide must be implementated using the Rest assured  methods only!");
		assertEquals(customResponse.getStatusCode(), 200);
	}
