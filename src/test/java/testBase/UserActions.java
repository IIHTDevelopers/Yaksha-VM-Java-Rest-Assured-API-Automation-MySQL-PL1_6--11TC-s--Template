...
public class UserActions
{
	...
	public UserActions waitTillPageLoad(By by, int seconds, String pageName) throws Exception
	{
		...
		while(count<=seconds)
		{
			ajaxIsComplete = ((JavascriptExecutor) driver).executeScript("return document.readyState")
					.toString()
					.equals("complete");
			if(ajaxIsComplete)
			{
		...
		do {
			try {
				...
				else if(locatorType.equalsIgnoreCase("cssSelector"))
				{
					...
					JavascriptExecutor jsExecutor = (JavascriptExecutor) driver;
					totalCount = Long.parseLong(jsExecutor.executeScript(path).toString());
				}
	...
	{
		Map<String, Long> timeDifferences = new HashMap<>();
